...
注意：请使用当前年份 (${currentDate}) 的最新 API 和语法。`;
   ...
   *
   * 路由策略：
   * 1. 需要网络搜索的任务 → 普通端点（支持 web_search）
   * 2. 代码/纯文本任务 → Coding Plan 端点（使用套餐额度，节省费用）
   */
  private selectGLEndpoint(content: string): {
    ...
    const searchPatterns = [
      // 中文搜索关键词
      /\b(搜索|搜|查|查找|查询|百度|谷歌|google)\b/i,
      /\b(最新|最近|今天|今日|news|新闻|资讯)\b/i,
      /\b(当前时间|现在几点|今天日期|今天是几号)\b/i,
      /\b(价格|报价|汇率|股价|天气)\b/i,
      // 英文搜索关键词
      /\b(search|find|look for|check|google)\b/i,
    ...
    const codingPatterns = [
      /\b(代码|code|函数|function|类|class|API)\b/i,
      /\b(写代码|生成代码|实现|开发|编程)\b/i,
      /\b(typescript|javascript|python|java|go|rust)\b/i,
      /\b(debug|调试|修复|fix|bug)\b/i,
    ];

    const lowerContent = content.toLowerCase();

    // 1. 优先判断：是否包含搜索类工具（需要网络搜索）
    const hasSearchTools = this.currentSkill?.availableTools?.some(t =>
      t.includes('search') || t.includes('read') || t.includes('fetch')
    );
    ...
    // 2. 判断是否包含搜索关键词
    const needsSearch = searchPatterns.some(p => p.test(content));
    if (needsSearch) {
      return {
        baseUrl: STANDARD_URL,
        endpointType: 'standard',
    ...
    // 3. 判断是否为编码任务
    const isCodingTask = codingPatterns.some(p => p.test(content));
    if (isCodingTask) {
      return {
    ...
    // 4. 默认：使用 Coding Plan 端点（节省费用）
    return {
      baseUrl: CODING_PLAN_URL,
      endpointType: 'coding-plan',
      needsWebSearch: false,
      reason: '默认使用 Coding Plan（节省费用）',
    };
  }
