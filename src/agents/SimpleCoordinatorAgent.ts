...
注意：请使用当前年份 (${currentDate}) 的最新 API 和语法。`;
  ...
  private selectGLEndpoint(content: string): {
    ...
    // ========== 优先级 3：检查输入内容（明确的搜索指令）==========
    const webSearchPatterns = [
      // 明确的网页搜索指令
      /\b(搜索|搜|搜一下|百度|谷歌|google)\b/i,
      // 实时新闻/资讯类（需要组合关键词）
      /\b(最新|recent|latest)\b.*(新闻|news|资讯|消息|动态|报道)\b/i,
      /\b(新闻|news|资讯)\b.*(最新|recent|latest|今天|今日)\b/i,
      // 实时信息类（需要明确的时间/实时词）
      /\b(当前时间|现在几点|今天日期|今天是几号)\b/i,
      /\b(天气|temperature|weather)\b.*(今天|今日|current|now)\b/i,
      /\b(价格|报价|汇率|股价|price|rate)\b.*(最新|实时|real-time|live)\b/i,
      // 明确的 URL 获取
      /\b(提取|抓取|获取|解析)\b.*URL/i,
      /https?:\/\/\S+/i,
    ];
...
`;
  ...
  private async callLLM(
    ...
    try {
      ...
      const routeDecision = this.selectGLEndpoint(content);
