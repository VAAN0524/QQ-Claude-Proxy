...
export class SimpleCoordinatorAgent implements IAgent {
  ...
  private async identifySkill(content: string): Promise<string | null> {
    ...
    const skillChecks: Array<{
      skill: string;
      keywords: string[];
      requireFile?: boolean;  // 是否需要文件引用
      excludePatterns?: string[];  // 排除模式
    }> = [
      {
        skill: 'smart_search',
        keywords: ['搜索', 'search', '查找', 'find', '资讯', '新闻', '消息', '最新']
      },
      {
        skill: 'smart_code',
        keywords: ['代码', '编程', 'code', '函数', '类', '脚本', '算法', '实现', '开发']
      },
      {
        skill: 'file',
      ...
      {
        skill: 'data',
        keywords: ['数据', '分析', 'data', '统计', '图表', 'csv', 'json', 'excel', 'xlsx'],
        requireFile: true,  // 需要文件引用或明确提到文件类型
        excludePatterns: ['局势', '战争', '政治', '国际', '军事', '经济形势', '市场分析']  // 排除一般性分析
      },
    ];

    for (const { skill, keywords, requireFile, excludePatterns } of skillChecks) {
      // 检查是否匹配关键词
      let matched = false;
      for (const keyword of keywords) {
        if (lowerContent.includes(keyword)) {
          matched = true;
          logger.debug(`[SimpleCoordinator] 关键词匹配: ${skill} (关键词: ${keyword})`);
          break;
        }
      }
      ...
      // 检查排除模式
      if (excludePatterns) {
        const shouldExclude = excludePatterns.some(pattern => lowerContent.includes(pattern));
        if (shouldExclude) {
          logger.debug(`[SimpleCoordinator] 跳过 ${skill}：匹配到排除模式`);
          continue;
        }
      }
      ...
      if (requireFile) {
        ...
        const fileIndicators = [
          '.csv', '.json', '.xlsx', '.xls', '.txt', '.md',
          '文件', '这个文件', '上传', '附件'
        ];
        const hasFileIndicator = fileIndicators.some(indicator => lowerContent.includes(indicator));

        if (!hasFileIndicator) {
          logger.debug(`[SimpleCoordinator] 跳过 ${skill}：未检测到文件引用`);
          continue;
        }
      }

      logger.info(`[SimpleCoordinator] 识别技能: ${skill}`);
      return skill;
    }
...
**注意**：图片/视频分析会自动进行，你只需要基于分析结果回答即可。`,
    ...
    if (lines[0] === '---') {
      ...
      if (yamlContent.includes('availableTools:')) {
        ...
        for (const yamlLine of yamlLines) {
          // 检查是否进入 availableTools 字段
          if (yamlLine.trim().startsWith('availableTools:')) {
            inToolsSection = true;
            continue;
          }
          if (inToolsSection) {
            const trimmed = yamlLine.trim();
            // 检查是否遇到新的 YAML 字段（不以 - 开头且包含冒号）
  ...
  private async executeWithTools(content: string, context: AgentContext, images: import('./base/Agent.js').Attachment[] = []): Promise<string> {
    ...
    // ========== 🎯 特殊任务处理 ==========
    // 以下任务需要特殊预处理，其他所有任务由 Function Calling 处理

    // 1. GitHub URL 处理（需要预处理 URL）
    const githubUrlMatch = content.match(/(https?:\/\/github\.com\/[^\s]+)/);
    if (githubUrlMatch) {
      return await this.executeGitHubTask(content, githubUrlMatch[1]);
    }

    // 2. 文件发送请求（完全由 LLM 判断，避免硬编码规则）
    // 使用 LLM 判断这是否是文件相关请求，避免关键词误判
    const fileIntent = await this.parseFileIntent(content);
    if (fileIntent.isFileRelated) {
      return await this.executeFileTask(content, fileIntent);
    }

    // ========== 🤖 Function Calling 处理所有其他任务 ==========
    // 搜索、代码、文件列表等任务均由 Function Calling 自动路由
    // SKILL.md 中的 availableTools 配置决定可用工具列表
    // 这样可以充分利用 LLM 的智能判断，避免硬编码关键字匹配
...
${result.content.substring(0, 3000)}${result.content.length > 3000 ? '\n\n...(内容已截断)' : ''}`;
  ...
  /**
   * 执行文件任务（发送、列表等）
   * @param content 用户原始消息
   * @param intent 已解析的意图对象
   */
  private async executeFileTask(
    content: string,
    intent: {
      isFileRelated: boolean;
      action: 'list' | 'send' | 'unknown';
      fileType?: 'image' | 'video' | 'document' | 'any';
      fileName?: string;
      selectionMode?: 'specific' | 'random' | 'latest';
      count?: number;
    }
  ): Promise<string> {
    logger.info(`[SimpleCoordinator] 文件任务: ${content.substring(0, 50)}`);
    logger.info(`[SimpleCoordinator] 解析意图: ${JSON.stringify(intent)}`);
    ...
    if (intent.action === 'list') {
      return this.formatFileList(allFiles, intent.fileType);
    }

    // 意图2: 发送文件
    if (intent.action === 'send') {
      return this.handleFileSend(intent, allFiles, workspacePath);
    }

    // 默认：返回文件列表
    return this.formatFileList(allFiles);
  }

  /**
   * 使用 LLM 智能解析意图，判断是否为文件相关请求
   */
  private async parseFileIntent(content: string): Promise<{
    isFileRelated: boolean;  // LLM 判断这是否是文件相关请求
    action: 'list' | 'send' | 'unknown';
    fileType?: 'image' | 'video' | 'document' | 'any';
    fileName?: string;
    selectionMode?: 'specific' | 'random' | 'latest';
    ...
    try {
      const prompt = `判断用户请求是否与本地文件操作相关，并提取意图信息。

用户消息：${content}

请以 JSON 格式返回：
{
  "isFileRelated": true/false（是否与本地文件操作相关）,
  "action": "list" 或 "send"（仅在 isFileRelated=true 时有效）,
  "fileType": "image/video/document/any（可选）",
  "fileName": "具体文件名（如果有）",
  "selectionMode": "specific/random/latest",
  "count": 数量（可选）
}
...
判断规则：
- isFileRelated = true: 用户想查看/发送本地工作区的文件（图片、视频、文档等）
- isFileRelated = false: 用户想搜索网络内容、获取资讯、查询信息等

文件操作示例：
- "有哪些图片" → isFileRelated: true, action: "list", fileType: "image"
- "发一个视频给我" → isFileRelated: true, action: "send", fileType: "video"
- "最新的文档" → isFileRelated: true, action: "list", fileType: "document", selectionMode: "latest"

非文件操作示例：
- "ai科技方面的劲爆消息有哪些" → isFileRelated: false（这是搜索新闻资讯）
- "最新的科技资讯" → isFileRelated: false（这是搜索网络内容）
- "武汉天气怎么样" → isFileRelated: false（这是查询实时信息）
...
只返回 JSON，不要其他内容。`;
      ...
      if (!apiKey) {
        // 降级到简单规则匹配
        return this.fallbackIntentParsing(content);
      }
      ...
      if (jsonMatch) {
        ...
        return {
          isFileRelated: parsed.isFileRelated === true,
          action: parsed.action || 'unknown',
          fileType: parsed.fileType,
          fileName: parsed.fileName,
          selectionMode: parsed.selectionMode || 'specific',
      ...
      logger.debug(`[SimpleCoordinator] LLM 意图识别失败，使用降级方案: ${error}`);
      return this.fallbackIntentParsing(content);
    }
  }
   ...
   * 降级方案：使用规则匹配解析意图
   */
  private fallbackIntentParsing(content: string): {
    isFileRelated: boolean;
    action: 'list' | 'send' | 'unknown';
    fileType?: 'image' | 'video' | 'document' | 'any';
    fileName?: string;
    ...
    // 判断是否为文件相关请求
    // 必须同时包含：动作词 + 文件类型词
    const actionKeywords = ['发', '传', '送', '列表', '查看'];
    const fileTypeKeywords = ['图片', '视频', '文档', '文件', '照片', 'image', 'video', 'doc', 'file'];
    const hasAction = actionKeywords.some(k => lowerContent.includes(k));
    const hasFileType = fileTypeKeywords.some(k => lowerContent.includes(k));
    const isFileRelated = hasAction && hasFileType;

    // 判断动作类型
    let action: 'list' | 'send' | 'unknown' = 'unknown';
    if (/列表|有哪些|有什么|查看|哪些/.test(lowerContent)) {
      action = 'list';
    } else if (/发给我|传给我|发送|送给我/.test(lowerContent)) {
      action = 'send';
    }

    // 判断文件类型
    let fileType: 'image' | 'video' | 'document' | 'any' = 'any';
    if (/图片|照片|image|img|png|jpg|jpeg/.test(lowerContent)) {
      fileType = 'image';
    } else if (/视频|video|mp4|avi|mov/.test(lowerContent)) {
      fileType = 'video';
    } else if (/文档|doc|txt|pdf|md/.test(lowerContent)) {
      fileType = 'document';
    }

    // 判断选择模式
    let selectionMode: 'specific' | 'random' | 'latest' = 'specific';
    if (/随便|任意|来个/.test(lowerContent)) {
      selectionMode = 'random';
    } else if (/最新|最近|latest/.test(lowerContent)) {
      selectionMode = 'latest';
    }

    // 提取文件名（简单匹配）
    let fileName: string | undefined;
    const fileMatch = content.match(/([a-zA-Z0-9_]+\.(jpg|png|jpeg|mp4|avi|mov|pdf|txt|md|docx?))/);
    if (fileMatch) {
    ...
    // 提取数量
    let count = 1;
    const countMatch = content.match(/(\d+)个?/);
    if (countMatch) {
  ...
  private async handleFileSend(
    ...
    // 根据意图选择文件
    const selectedFiles = this.selectFiles(intent, allFiles, workspacePath);
    ...
    return `✅ 已将 ${selectedFiles.length} 个文件添加到发送队列:\n${selectedFiles.map(f => `  - ${path.basename(f)}`).join('\n')}`;
  }
  ...
  private selectFiles(
    ...
    // 1. 按文件类型过滤
    if (intent.fileType && intent.fileType !== 'any') {
      const extMap: Record<string, string[]> = {
        image: ['.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp'],
        video: ['.mp4', '.avi', '.mov', '.mkv', '.webm'],
        document: ['.md', '.txt', '.json', '.pdf', '.docx', '.xlsx', '.doc', '.xls'],
      };
      const exts = extMap[intent.fileType] || [];
    ...
    if (intent.fileName) {
      const matched = candidates.filter(f => {
        ...
        return baseName.includes(intent.fileName.toLowerCase());
      });
      if (matched.length > 0) {
        return matched.map(f => path.join(workspacePath, f));
      }
    }

    // 3. 按选择模式排序
    if (intent.selectionMode === 'latest') {
      candidates = candidates.sort((a, b) => {
        try {
          const statA = statSync(path.join(workspacePath, a));
          const statB = statSync(path.join(workspacePath, b));
          return statB.mtimeMs - statA.mtimeMs;
        } catch {
          return 0;
        }
      });
    } else if (intent.selectionMode === 'random') {
      // 随机打乱
      candidates = candidates.sort(() => Math.random() - 0.5);
    }

    // 4. 选择指定数量
    const count = Math.min(intent.count || 1, candidates.length);
    const selected = candidates.slice(0, count);

    return selected.map(f => path.join(workspacePath, f));
  }

  /**
   * 格式化文件列表
   */
  private formatFileList(files: string[], filterType?: string): string {
    ...
    if (filterType) {
      const extMap: Record<string, string[]> = {
        image: ['.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp'],
        video: ['.mp4', '.avi', '.mov', '.mkv', '.webm'],
        document: ['.md', '.txt', '.json', '.pdf', '.docx', '.xlsx'],
      };
      const exts = extMap[filterType] || [];
    ...
    if (filteredFiles.length === 0) {
      return `📁 工作区为空${filterType ? `（${filterType}）` : ''}，没有文件。`;
    }

    return `📁 **工作区文件**${filterType ? `（${filterType}）` : ''} (${filteredFiles.length} 个):\n${filteredFiles.map(f => `  - ${f}`).join('\n')}\n\n💡 提示: 使用 "把 xxx 发给我" 来接收文件`;
  }
...
注意：请使用当前年份 (${currentDate}) 的最新 API 和语法。`;
  ...
  /**
   * 智能路由：根据任务类型选择 GLM 端点
   *
   * 路由策略（严格执行）：
   * - 网页搜索/实时新闻/资讯 → 普通端点 + web_search
   * - 当前技能是搜索类 → 普通端点 + web_search
   * - 其他所有情况（代码、日常对话等）→ Coding Plan 端点
   */
  private selectGLEndpoint(content: string): {
    baseUrl: string;
    endpointType: 'coding-plan' | 'standard';
    needsWebSearch: boolean;
    reason: string;
  } {
    // 定义端点
    const CODING_PLAN_URL = 'https://open.bigmodel.cn/api/coding/paas/v4';
    const STANDARD_URL = 'https://open.bigmodel.cn/api/paas/v4';

    // ⚠️ 判断优先级：
    // 1. 当前技能是搜索类技能（用户明确需要搜索）
    // 2. 输入内容包含明确的搜索关键词
    // 3. 默认使用 Coding Plan

    // ========== 优先级 1：检查当前技能 ==========
    const currentSkillName = this.currentSkill?.name?.toLowerCase() || '';
    const searchSkills = ['smart_search', 'smart-search', 'search', 'ducksearch', 'tavily-search', 'web-search'];

    if (searchSkills.some(s => currentSkillName.includes(s))) {
      logger.debug(`[SimpleCoordinator] 检测到搜索技能: ${this.currentSkill?.name}`);
      return {
        baseUrl: STANDARD_URL,
        endpointType: 'standard',
        needsWebSearch: true,
        reason: `当前技能为搜索类 (${this.currentSkill?.name})`,
      };
    }

    // ========== 优先级 2：检查技能可用工具 ==========
    const hasSearchTools = this.currentSkill?.availableTools?.some(t =>
      t.includes('search') || t.includes('tavily') || t === 'exa_search' || t === 'exa_code_search'
    );

    if (hasSearchTools) {
      logger.debug(`[SimpleCoordinator] 技能包含搜索工具`);
      return {
        baseUrl: STANDARD_URL,
        endpointType: 'standard',
        needsWebSearch: true,
    ...
    // ========== 优先级 3：检查输入内容（放宽搜索触发条件）==========
    const webSearchPatterns = [
      // ===== 明确的网页搜索指令 =====
      /\b(搜索|搜|搜一下|百度|谷歌|google)\b/i,
      /\b(帮我找|帮我查|帮我看看)\b/i,

      // ===== 实时新闻/资讯类（放宽条件）=====
      /\b(最新|recent|latest|当前|current)\b.*(新闻|news|资讯|消息|动态|报道|情况)\b/i,
      /\b(新闻|news|资讯|消息|动态)\b.*(最新|recent|latest|今天|今日|当前)\b/i,

      // ===== 信息查询类（放宽条件）=====
      /\b(天气|temperature|weather)\b/i,
      /\b(价格|报价|汇率|股价|price|rate)\b/i,
      /\b(怎么样|如何|what|how|是什么|what's)\b/i,

      // ===== 知识/事实类问题 =====
      /\b(什么是|什么叫|哪个|哪些|谁|who|what|which)\b/i,
      /\b(你知道|告诉我|解释|explain|tell me)\b/i,

      // ===== 明确的 URL/网页操作 =====
      /\b(提取|抓取|获取|解析|访问|打开)\b.*(URL|url|网页|链接|网站)\b/i,
      /https?:\/\/\S+/i,

      // ===== 技术相关 =====
      /\b(怎么用|如何使用|how to use|教程|tutorial|文档|documentation|API)\b/i,

      // ===== 数据/统计类 =====
      /\b(数据|统计|data|stats|排名|rank)\b/i,

      // ===== 政治/军事/局势分析类（新增）=====
      /\b(局势|战争|冲突|政治|国际|军事|经济|市场|趋势|形势)\b.*(分析|最新|当前|如何|怎么样)\b/i,
      /\b(分析|最新|当前)\b.*(局势|战争|冲突|政治|国际|军事|经济|市场|趋势|形势)\b/i,
      /\b(战火|冲突|局势)\b.*(蔓延|持续|发展|变化)\b/i,
    ];

    const needsWebSearch = webSearchPatterns.some(p => p.test(content));
    ...
    if (needsWebSearch) {
      return {
        ...
        endpointType: 'standard',
        needsWebSearch: true,
        reason: '检测到网页搜索/实时新闻/资讯请求',
      };
    }

    // ========== 优先级 4：默认 Coding Plan ==========
    return {
      baseUrl: CODING_PLAN_URL,
      endpointType: 'coding-plan',
      needsWebSearch: false,
      reason: '默认使用 Coding Plan（代码/日常对话，节省费用）',
    };
  }
  ...
  private async callLLM(
    ...
    try {
      ...
      if (videos.length > 0 && this.mcpClient) {
        ...
        const prompt = userQuery || '请详细分析这个视频的内容，包括主题、关键信息、场景和主要观点。';
      ...
      if (imagesOnly.length > 0 && this.mcpClient) {
        ...
        const prompt = userQuery || '请详细描述这张图片的内容，包括主要元素、颜色、布局和任何可见的文字。';
      ...
      // 🧠 智能路由：根据任务类型自动选择端点
      // Coding Plan 端点：代码生成任务，使用套餐额度（节省费用）
      // 普通端点：需要网络搜索的任务，支持 web_search 工具
      const routeDecision = this.selectGLEndpoint(userQuery);
      const baseUrl = routeDecision.baseUrl;

      logger.info(`[SimpleCoordinator] 智能路由: ${routeDecision.reason} → ${routeDecision.endpointType}`);
      if (routeDecision.needsWebSearch) {
        logger.info('[SimpleCoordinator] 将启用智谱 AI 内置网络搜索');
      }
      ...
      // ========== 🎯 当前查询优先指令 ==========
      // 确保 LLM 始终优先处理最新的用户查询，即使对话历史中有多个话题
      systemPrompt += '\n\n## 🎯 重要指令\n\n';
      systemPrompt += '**你必须始终优先处理用户最新的查询请求。**\n';
      systemPrompt += '当调用搜索工具或其他工具时，必须基于当前最新的用户问题提取关键词，\n';
      systemPrompt += '而不是重复对话历史中的旧话题。\n';
      systemPrompt += '如果用户提出了新的问题或新的话题，即使历史对话中包含其他话题，\n';
      systemPrompt += '你也必须完全专注于当前的新问题。\n';
      ...
      for (let iteration = 0; iteration < maxIterations; iteration++) {
        ...
        // ========== 🎯 当前查询高亮 ==========
        // 每次迭代都重新构建消息数组，确保当前查询高亮始终在最后
        const messagesForRequest = [
          ...messages,
          {
            role: 'system' as const,
            content: `🎯 【当前用户查询】\n用户最新的问题是：${userQuery}\n\n请基于这个最新问题来决定是否调用工具以及调用工具时的参数。\n忽略历史对话中的旧话题，专注于当前的新问题。`,
          },
        ];
        ...
        // 添加智谱 AI 内置网络搜索工具（仅普通端点支持）
        // 根据智能路由决策判断是否需要添加 web_search
        if (!isLastIteration && routeDecision.needsWebSearch) {
          ...
          logger.debug('[SimpleCoordinator] 已添加智谱 AI 内置 web_search 工具');
        } else if (!routeDecision.needsWebSearch) {
        ...
        if (toolCalls.length > 0) {
          ...
          for (const toolCall of toolCalls) {
            ...
            try {
              ...
              const tool = this.toolManager.get(toolName);
              if (tool) {
                // 使用 ToolCallTracker 追踪工具调用
                toolResult = await ToolCallTracker.track({
                  toolName,
                  parameters: toolArgs,
                  execute: async () => tool.execute(toolArgs),
                }) as string;
              } else {
      ...
      // ========== 🔄 回答质量自检和闭环反馈 ==========
      const qualityCheck = this.checkResponseQuality(finalResponse, userQuery);
      if (!qualityCheck.isQualityGood) {
        ...
        // 尝试自动纠正
        const correctedResponse = await this.autoCorrectResponse(userQuery, finalResponse, qualityCheck);
