/**
 * 知识库记忆管理 - 类型定义
 * 全局Skill类型定义，供所有Claude Code会话使用
 */
...
export interface SaveOptions {
  ...
  enableSummary?: boolean;       // 是否生成摘要（Phase 2）
}
...
export interface SearchQuery {
  ...
  limit?: number;                // 结果数量限制
  useSemantic?: boolean;         // 是否使用语义搜索（Phase 2）
  minSimilarity?: number;        // 最小相似度阈值（Phase 2）
}
...
export interface KnowledgeServiceConfig {
  ...
  enableSemantic?: boolean;      // 是否启用语义搜索（Phase 2）
}
...
export interface SemanticSearchResult {
  ...
  matchType: 'exact' | 'semantic' | 'keyword'; // 匹配类型
}

// Phase 2: 自然语言意图
export interface Intent {
  type: 'save' | 'search' | 'get' | 'list' | 'tags' | 'stats' | 'delete' | 'update' | 'unknown';
